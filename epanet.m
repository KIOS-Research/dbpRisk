...
for i=1:value.LinkCount
    ...
    h(:,4)=line([x1 NodeCoordinates{3}{i} x2],[y1 NodeCoordinates{4}{i} y2],'LineWidth',1,'Parent',axesid);
    ...
    elseif length(hh) && ~isempty(selectColorLink)
       ...
       if tt>1
            ...
            line([x1 NodeCoordinates{3}{i} x2],[y1 NodeCoordinates{4}{i} y2],'LineWidth',2,'Color',char(selectColorLink(hh)),'Parent',axesid);
        end
        text((x1+x2)/2,(y1+y2)/2,value.LinksAll(i),'Fontsize',fontsize,'Parent',axesid);
    end
...
function value=InputFileInfo(obj)
    ...
    if value.UScustomary==1;
        ...
        value.LinkPipeDiameterUnits='in';
        value.NodeTankDiameterUnits='ft';
        value.EnergyEfficiencyUnits='percent';
        value.NodeElevationUnits='ft';
        ...
        value.NodeHeadUnits='ft';
        value.LinkLengthUnits='ft';
        ...
        value.QualitySourceMassInjectionUnits='mass/min';
        value.LinkVelocityUnits='ft/sec';
        value.NodeTankVolumeUnits='cubic feet';
        value.QualityWaterAgeUnits='hours';
    else % SI Metric
        value.PatternDemandsUnits=value.LinkFlowUnits;
        value.LinkPipeDiameterUnits='mm';
        value.NodeTankDiameterUnits='m';
        value.EnergyEfficiencyUnits='percent';
        value.NodeElevationUnits='m';
        value.NodeEmitterCoefficientUnits='flow units @ 1 meter drop';
        value.EnergyUnits='kwatt-hours';
        value.LinkFrictionFactorUnits='unitless';
        value.NodeHeadUnits='m';
        value.LinkLengthUnits='m';
        value.LinkMinorLossCoeffUnits='unitless';
        value.LinkPumpPowerUnits='kwatts';
        value.QualityReactionCoeffBulkUnits='1/day (1st-order)';
        value.QualityReactionCoeffWallUnits='mass/sq-m/day(0-order), m/day (1st-order)';
        value.LinkPipeRoughnessCoeffUnits='mm(Darcy-Weisbach), unitless otherwise';
        value.QualitySourceMassInjectionUnits='mass/minute';
        value.LinkVelocityUnits='m/sec';
        value.NodeTankVolumeUnits='cm';
        value.QualityWaterAgeUnits='hours';
    end
end
...
for i=1:obj.LinkCount
    ...
    % Show Link id
    if (strcmp(lower(Link),'yes') && ~length(hh))
        text((x1+x2)/2,(y1+y2)/2,obj.LinkNameID(i),'Fontsize',fontsize,'Parent',axesid);
    end
...
for i=1:obj.NodeCount
    ...
    % Show Node id
    if (strcmp(lower(Node),'yes') && ~length(hh))
        text(x,y,obj.NodeNameID(i),'Fontsize',fontsize,'Parent',axesid);%'BackgroundColor',[.7 .9 .7],'Margin',margin/4);
    end
