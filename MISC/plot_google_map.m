...
if nargout <= 1 % plot map
    % display image
    h = image(lonVect,latVect,uniImag,'Parent',varargin{1}.previousg);%mariosmsk
    set(h,'CDataMapping','scaled');%mariosmsk
    set(gca,'YDir','Normal')
    set(h,'tag','gmap')
    ...
    zoomHandle = zoom; load([pwd,'\RESULTS\','map.1'],'handles','-mat');%%mariosmsk    
    panHandle = pan;
    if autoRferesh && handles.MapOn==0 
